...
使用方法：把完整的 `Codex 主题编辑器.zip`、客户喜欢的主题图片（可选）和下面整段提示词，一起发送给客户自己的 macOS Codex。不要拆分 ZIP 内的 Skill 目录。
...
2. 解压后的客户目录根部应能看到：
   ...
   - `使用说明.txt`
   - `给 Codex 的部署提示词.md`
   完整 Skill 位于 `Codex Dream Skin Skill` 目录。不要删除、改名或只复制其中的 CSS/图片。

3. 将完整 Skill 记为 `<SKILL>`，先完整阅读：
   - `<SKILL>/README.md`
   - `<SKILL>/SKILL.md`
   - `<SKILL>/references/qa-inventory.md`
...
5. 如果我上传了主题图片，使用安装后的脚本处理素材：
   `~/.codex/skills/codex-dream-skin/scripts/customize-theme-macos.sh --image "<图片绝对路径>" --name "我的 Codex Dream Skin" --no-apply`
...
7. 启动后必须运行：
   - `~/.codex/skills/codex-dream-skin/scripts/doctor-macos.sh --require-live`
   - `~/.codex/skills/codex-dream-skin/scripts/verify-dream-skin-macos.sh --reload --screenshot "<首页验收截图绝对路径>"`
